...
import { DEFAULT_REWARDS_LOGIN_TOKEN, REWARDS_LOGIN_TOKEN_ID } from "../constants";
...
export async function signUp(email: string, password: string, params: SignUpParams): Promise<boolean> {
    let respBody: any = {
        "email": email,
        "password": password,
        "feature": "missing",
    ...
    try {
        const resp = await fanmakerPostRequest("https://api.fanmaker.com/api/v2/registration", DEFAULT_REWARDS_LOGIN_TOKEN, JSON.stringify(respBody));
        ...
        console.error(e);
        if (e instanceof Error) throw new Error("Unable to make new account due to " + e.message + " " + e.stack);
